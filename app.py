...
# 定義マップ
# =========================
TYPE_MAP = {"A": "オリジナル（伴奏有）", "B": "オリジナル（無伴奏）", "C": "アレンジ", "D": "特殊"}
...
with col3:
    st.write("声部（複数選択可）")
    part_inputs = st.multiselect("", part_list, default=[])

# 区分（横並び）
st.write("区分")
type_input = st.radio("", ["指定しない"] + type_list, horizontal=True)
...
if filtered_df.empty:
    ...
    st.dataframe(
        ...
        column_config={"url": st.column_config.LinkColumn("楽譜リンク", display_text="開く")}
    )
