...
if df_all.empty:
    ...
    for i, tab in enumerate(tabs):
        ...
        safe_folder = re.sub(r"\W+", "_", current_folder)  # ← ① 保険修正
        ...
        with tab:
            ...
            if len(existing_parts) > 1:
                ...
                master_part = f"all_part_{safe_folder}"
                ...
                def cb_part_sync():
                    st.session_state[master_part] = all(
                        st.session_state.get(f"part_{safe_folder}_{p}", False)
                        for p in existing_parts
                    )
            ...
            if len(existing_types) > 1:
                ...
                master_type = f"all_type_{safe_folder}"
                ...
                def cb_type_sync():
                    st.session_state[master_type] = all(
                        st.session_state.get(f"type_{safe_folder}_{t}", False)
                        for t in existing_types
                    )
